...
function getAssetName(token: string): string {
  const t = token.toLowerCase();
  if (t === NATIVE_TOKEN) return 'ETH';
  if (t === '0xd8e20462edce38434616cc6a6a560bb76b582ed8') return 'USDC';
  if (t === '0xaf88d065e77c8cc2239327c5edb3a432268e5831') return 'USDC';
  if (t === '0xfd086bc7cd5c481dcc9c85ebe478a1c0b69fcbb9') return 'USDT';
  if (t === '0xdac17f958d2ee523a2206206994597c13d831ec7') return 'USDT';
  return 'UNKNOWN';
}
...
function hex2amount(hex: string, decimals = 6): string {
...
function parseBridgeDepositInput(input: string): Record<string, string> | null {
  if (!input.startsWith(BRIDGE_DEPOSIT_SELECTOR)) return null;
...
function parseSpotDepositInput(input: string): Record<string, string> | null {
  if (!input.startsWith(SPOT_DEPOSIT_SELECTOR)) return null;
...
function parseSpotVaultDepositLog(log: { topics: string[]; data: string }): Record<string, string> | null {
  if (log.topics[0] !== TOPICS.SPOT_VAULT_DEPOSIT) return null;
  return {
...
function parseRelayDepositLog(log: { topics: string[]; data: string }): Record<string, string> | null {
  if (log.topics[0] !== TOPICS.RELAY_DEPOSIT_SUCCEEDED) return null;
...
function parseErc20TransferLog(log: { topics: string[]; data: string }): Record<string, string> | null {
  if (log.topics[0] !== TOPICS.ERC20_TRANSFER) return null;
  return {
    from: hexToAddress(log.topics[1] ?? ''),
...
async function getBlockTimestamp(rpcUrl: string, blockNumber: string): Promise<number | undefined> {
  try {
    const block = (await rpcCall(rpcUrl, 'eth_getBlockByNumber', [blockNumber, false])) as { timestamp?: string } | null;
    if (block?.timestamp) return Number(BigInt(block.timestamp));
  } catch { /* ignore */ }
  return undefined;
}
...
export function getDefaultChains(edgeChainRpcUrl: string): ChainConfig[] {
  return [
    {
      name: 'Edge Chain',
      chainId: 3343,
...
export async function trackDeposit(
  ...
  const results = await Promise.allSettled(
    chains.map(async (chain) => {
      const [receipt, tx] = await Promise.all([
        rpcCall(chain.rpcUrl, 'eth_getTransactionReceipt', [hash]) as Promise<TxReceipt | null>,
        rpcCall(chain.rpcUrl, 'eth_getTransactionByHash', [hash]) as Promise<TxData | null>,
      ]);
  ...
  let matched: { chain: ChainConfig; receipt: TxReceipt; tx: TxData | null } | null = null;
  let foundPending: ChainConfig | null = null;
