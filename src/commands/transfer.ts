...
export function registerTransferCommand(program: Command): void {
  ...
  transfer
    ...
    .action(async (coinId: string, _opts: unknown, cmd: Command) => {
      try {
        await init();
  ...
  transfer
    ...
    .action(async (
      ...
      opts: { reason: string; extraType?: string; extraData?: string },
      cmd: Command,
    ) => {
      try {
        await init();
        ...
        const meta = await client.getMetaData();
        const globalData = meta as unknown as Record<string, unknown>;
        const global = globalData.global as Record<string, unknown> | undefined;
        const collateralCoin = global?.starkExCollateralCoin as Record<string, string> | undefined;
        ...
        const l2 = computeTransferL2Fields(
          starkPrivateKey,
        ...
        if (opts.extraType) body.extraType = opts.extraType;
        if (opts.extraData) body.extraDataJson = opts.extraData;
    ...
    .action(async (opts: { size: string }, cmd: Command) => {
      try {
        ...
        output(fmt, data, () => {
          ...
          printTable(
            ['Transfer ID', 'Coin', 'Amount', 'Receiver', 'Status', 'Time'],
            list.map(t => [
    ...
    .action(async (opts: { size: string }, cmd: Command) => {
      try {
        ...
        output(fmt, data, () => {
          ...
          printTable(
            ['Transfer ID', 'Coin', 'Amount', 'Sender', 'Status', 'Time'],
            list.map(t => [
              String(t.transferId ?? t.id ?? ''),
              String(t.coinId ?? ''),
              String(t.amount ?? ''),
  ...
  transfer
    ...
    .action(async (
      amount: string,
      ethAddress: string,
      opts: { chainId: string; coin: string },
      cmd: Command,
    ) => {
      try {
        await init();
  ...
  transfer
    ...
    .action(async (direction: string, ids: string, _opts: unknown, cmd: Command) => {
      try {
        await init();
        const fmt = getFormat(cmd);
