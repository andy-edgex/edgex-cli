...
import { computeL2OrderFields } from './l2-signer.js';
export function getL2Meta(contract, coins) {
...
export function buildOrderPayload(params) {
    ...
    if (tp) {
        ...
        const tpL2 = computeL2OrderFields({ side: tpSide, type: 'MARKET', size, oraclePrice: tpSide === 'BUY' ? tp : undefined, accountId }, l2Meta, starkPrivateKey);
    ...
    if (sl) {
        ...
        const slL2 = computeL2OrderFields({ side: slSide, type: 'MARKET', size, oraclePrice: slSide === 'BUY' ? sl : undefined, accountId }, l2Meta, starkPrivateKey);
