...
function requireSymbol(symbol) {
    if (!symbol)
        throw new EdgexError('Symbol is required');
    const contract = resolveSymbol(contracts, symbol);
    if (!contract)
        throw new EdgexError(`Unknown symbol: ${symbol}. Use "edgex market ticker" to list available contracts.`);
    return contract;
}
...
export function registerMarketCommand(program) {
    ...
    market
        ...
        try {
            ...
            output(fmt, data, () => {
                ...
                printTable(['Symbol', 'Last Price', 'Change 24h', 'High', 'Low', 'Volume', 'OI'], data.map(t => [
                    t.contractName,
    ...
    market
        ...
        try {
            ...
            if (!klineType)
                throw new EdgexError(`Invalid interval: ${opts.interval}. Valid: ${Object.keys(KLINE_INTERVALS).join(', ')}`);
            ...
            output(fmt, data, () => {
                ...
                printTable(['Time', 'Open', 'High', 'Low', 'Close', 'Volume (USD)'], bars.map(b => [
    ...
    market
        ...
        try {
            ...
            output(fmt, data, () => {
                ...
                printTable(['Contract ID', 'Funding Rate', 'Forecast', 'Previous', 'Timestamp'], data.map(f => {
                    const d = f;
                    return [
                        d.contractId ?? '',
    ...
    market
        ...
        try {
            ...
            output(fmt, data, () => {
                const d = data;
                const summary = (d.tickerSummary ?? d);
    ...
    market
        ...
        try {
            ...
            output(fmt, data, () => {
                ...
                printTable(['Exchange', 'Range', 'Long %', 'Short %', 'Buy Vol (USD)', 'Sell Vol (USD)'], list.map(r => [
                    r.exchange,
                    r.range,
