...
export function registerDepositStatusCommand(program) {
    program
        ...
        try {
            ...
            output(fmt, result, () => {
                ...
                if (result.blockNumber)
                    pairs.push(['Block', String(result.blockNumber)]);
                if (result.amount)
                    pairs.push(['Amount', `${result.amount} ${result.asset ?? ''}`]);
                if (result.accountId)
                    pairs.push(['Account/Key', result.accountId]);
                if (result.to)
                    pairs.push(['Contract', result.to]);
                if (result.timestamp)
                    pairs.push(['Time', new Date(result.timestamp * 1000).toISOString()]);
...
//# sourceMappingURL=deposit-status.js.map