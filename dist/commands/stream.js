...
function connectPublic(channels) {
...
async function connectPrivate() {
    ...
    const { timestamp, signature } = signRequest('GET', path, config.starkPrivateKey, { accountId: config.accountId });
    const ws = new EdgexWebSocket({
...
export function registerStreamCommand(program) {
    ...
    stream
        ...
        try {
            ...
            if (!klineType)
                throw new EdgexError(`Invalid interval: ${opts.interval}`);
